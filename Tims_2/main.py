...
def show_emp(inputList):
    ...
    for i in range(len(elems) - 1):
        ...
        plt.plot((elems[i], elems[i + 1]), (cntSum / len(inputList), cntSum / len(inputList)), color='blue')
        plt.plot((elems[i], elems[i]), ((cntSum-cnts[i]) / len(inputList), (cntSum) / len(inputList)), color='blue')
    plt.show()
...
textfile_bt = Button(window, text='Read from file', command=fileInput)
...
lb_border_left = Label(window, text='Left Border: ')
lb_border_left.grid(row=1, column=0)

border_left_entry = Entry(window)
border_left_entry.grid(row=1, column=1)

lb_border_right = Label(window, text='Right Border: ')
lb_border_right.grid(row=2, column=0)

border_right_entry = Entry(window)
border_right_entry.grid(row=2, column=1)

lb_count = Label(window, text='Count of Elements: ')
lb_count.grid(row=3, column=0)
...
dia_chastot_but.grid(row=5, column=1)
...
pol_chastot_but.grid(row=5, column=2)
...
his_chastot_but.grid(row=5, column=3)
...
discrete_info_but = Button(window, text='Discrete Info', command=info)
discrete_info_but.grid(row=5, column=0)
...
interval_info_but = Button(window, text = 'Interval Info', command = interval_info)
interval_info_but.grid(row = 6, column =0 )

def emp():
    show_emp(data.lst)


emp_but = Button(window, text='Empirical Distribution Function', command=emp)
emp_but.grid(row=6, column=1)

discr_label = Label(window, text = '------№2------')
discr_label.grid(row = 8, column = 0)

def generateNormalDistr(elemCnt, n, var):
    distr = norm.pdf(np.arange(-var*3, var*3, 0.001), 0, var)
    s = len(distr)//n
    p = []
    for i in range(0, len(distr), s):
        p.append(sum(distr[i:(i+s)])/sum(distr))

    lst = []
    for i in range(n):
        lst.append([str(round(p[i],3)), str(round(elemCnt *p[i],3))])
    return lst
...
def disc_normal():
    ...
    ind = 0
    r = len(arr)-1
    for elem in generateNormalDistr(len(data.lst), len(arr), np.var(data.lst)):
...
discr_normal_but = Button(window, text = 'Discrete Normal', command = disc_normal)
discr_normal_but.grid(row = 9, column = 0)

def generatePoissonDistr(elemCnt, n, mean):
    distr = [poisson.pmf(i, mean) for i in range(0, max(15, int(mean)*2))]
    s = len(distr)//n
    p = []
    for i in range(0, len(distr), s):
        p.append(sum(distr[i:(i+s)])/sum(distr))
...
def disc_puason():
    ...
    ind = 0
    r = len(arr)-1
    for elem in generatePoissonDistr(len(data.lst), len(arr), float(np.mean(data.lst))):
        arr[ind].append(elem[0])
        arr[ind].append(elem[1])
        ind += 1
    headers = [["x_i", "m_i", "p_i", "n*p_i"]]
...
discr_puason_but = Button(window, text = 'Discrete Puasson`s', command = disc_puason)
discr_puason_but.grid(row = 9, column = 1)
...
unin_normal_but = Button(window, text = 'Unintervated Normal', command = unin_normal)
unin_normal_but.grid(row = 9, column = 2)
...
unin_puason_but = Button(window, text = 'Unintervated Puasson`s', command = unin_puason)
unin_puason_but.grid(row = 9, column = 3)
